...
def roman(number_to_convert):
    ...
    ones = i[number_to_convert % 10]
    tens = x[(number_to_convert % 100) // 10]
    hundreds = c[(number_to_convert % 1000) // 100]
    thousands = m[number_to_convert // 1000]

    number_converted = (thousands + hundreds +
                        tens + ones).replace(" ", "")
